...
| Product | ConfirmDelete/ConfirmKarmaExpense | `[~]` | Delete confirmations cover every collection (fixed the one gap found: Burn Edge now shows its own unconditional confirm, matching legacy). ConfirmKarmaExpense now covers Attributes/Skills/Initiation, career-mode Quality purchase/buy-off, and, newly, career-mode Spell learning (flat KarmaSpell), Complex Form learning (category/house-rule-dependent cost, plus the expense-log/undo entry it was previously missing), and Martial Art/Maneuver purchase (5*KarmaQuality / KarmaManeuver - legacy shows no confirm dialog for these two, but this port confirms them anyway for UI consistency with the other three). Focus/Stacked Focus binding already charged Karma correctly in Core; Stacked Focus gained a `GetStackedFocusBindingKarmaCost` preview method for parity with the regular-Focus preview. The Avalonia Gear tab now has Bind/Unbind Focus and Bind/Unbind Stacked Focus buttons wired to these Core methods (with the Karma confirm dialog on binding), closing the previously-flagged "Core is ready but nothing triggers it" gap. Not covered: creating a Stacked Focus from the UI (`CreateStackedFocus` needs a multi-select of 2+ unbonded Foci, which the current single-selection Gear tree doesn't support) - still Core-only. |
