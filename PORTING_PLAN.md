...
| Product | Native print | `[~]` | Linux smoke test passed manually (real printer + Virtual_PDF_Printer via CUPS, including in-sheet interactivity like page-break toggles). SheetPreviewDialog is now two synced windows: a small real-Avalonia-button toolbar (proven 100% reliable, unlike three different in-page HTML/JS bridge mechanisms tried and abandoned) and an Avalonia.Controls.WebView `NativeWebDialog` positioned/resized to follow it, holding the actual interactive sheet content - the only engine on this port's target hardware that renders *and* stays interactive, since embedding it as a child control renders completely blank on NVIDIA/GBM (confirmed even with the DMA-BUF workaround applied), and an off-screen-bitmap alternative (Ultralight, evaluated and reverted) has no interactivity at all. CefGlue.Avalonia was also evaluated and rejected - no release compatible with this project's Avalonia 12.1.0. Carries a `WEBKIT_DISABLE_DMABUF_RENDERER=1` re-exec workaround for the content window's own WebKitGTK engine (`Failed to create GBM buffer` on this Mesa/DRM driver combo otherwise) - see `Chummer.Avalonia/src/Program.cs`'s `ReExecWithWebKitGtkGbmWorkaroundOnLinux`. Windows and macOS smoke tests remain. |
...
| Product | LocalisedUpdatesOnly/OmaeAutoLogin | `[x]` | Both are vestigial legacy fields never wired to real behavior even in WinForms - `LocalisedUpdatesOnly` has no reader anywhere in legacy `frmUpdate.cs` (the GitHub-Releases-based `UpdateChecker` this port uses has no per-language artifacts to filter by anyway), and `OmaeAutoLogin`/`OmaeUserName`/`OmaePassword` had no UI at all in legacy (`frmOptions.Designer.cs` has zero "Omae" references) since Omae itself predates any exposed settings surface for it. `LocalisedUpdatesOnly` is already ported and persisted (`Chummer.Core/src/Options.cs`, `OptionsDialogViewModel.cs:157-163`) as the same cosmetic no-op legacy had. `OmaeAutoLogin` is moot - Omae was already fully removed and replaced by RunnersPoint.Api, whose `CloudDocumentsDialogViewModel.Auth.cs` already auto-restores a saved login/token on open (`HasStoredLogin()`), a strict functional superset of what an "auto-login" flag would have done. No further work needed. |
