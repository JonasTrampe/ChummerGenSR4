...
## Addendum (2026-07-19): Avalonia chosen; real port started; a bigger structural blocker found

The Mono-vs-Avalonia call was settled in Avalonia's favor after the spike (both risk-area prototypes above landed successfully). Work moved to branch `feature/linux-port` and the actual port began, starting with the foundation everything else depends on: a shared data/logic layer both the WinForms app and the Avalonia app can reference, instead of the spike's hand-mocked data.

**Done:**
- `Chummer.Core`, a new SDK-style project multi-targeting `net48;net8.0`, added to `ChummerGenSR4.sln` and referenced from `Chummer.csproj`.
- Moved the first fully-portable leaf files into it: `clsMTRNG.cs`, `clsRuntimeInfo.cs`, `clsSettingsStore.cs` (had to make their previously-`internal` types `public` for cross-assembly use).
- Fixed 3 of `clsCharacter.cs`'s WinForms leaks: `Application.ProductVersion` → `Assembly.GetEntryAssembly()` version, a `MessageBox.Show` on wrong-game-edition load replaced with a `LoadedWrongGameEdition` flag the caller checks, and a mugshot `Image.FromStream`/`Image.Save` GDI+ round-trip replaced with a direct byte-array write (it was decoding and re-encoding the image for no reason - the ".img" extension was always read back as raw bytes regardless of format anyway, per the code's own comment).

**Found, not yet solved - the real blocker:** a first attempt to drop `System.Windows.Forms` from `clsCharacter.cs` entirely surfaced that `TreeNode` (and in `clsEquipment.cs`/`clsUnique.cs`, `ContextMenuStrip` too) is threaded directly through **~21 `Create()` methods across the model layer** - `Quality`, `Spell`, `Metamagic`, `TechProgram`, `Art`, `MartialArtAdvantage`, `MartialArtManeuver`, `Power`, `Armor`, `ArmorMod`, `Cyberware`, `Weapon`, `WeaponAccessory`, `WeaponMount`, `Lifestyle`, `Gear`, `Vehicle` all build a UI tree node (and sometimes a context menu) as a *side effect* of constructing the business object, called from ~9 files (`frmCreate.cs`, `frmCareer.cs`, various selector dialogs). This is deeper and more pervasive than the file-level `.Designer.cs` risk audit above caught, since it's baked into the model's own construction methods, not just the UI layer.

This is now understood to be **the single largest remaining structural blocker** for a shared `Chummer.Core` data layer - bigger than the original "clean up a handful of `MessageBox.Show` calls" expectation. The fix is conceptually simple (split each `Create()` into pure object construction + a separate step where the caller builds the corresponding UI node from the result - basic MVVM separation) but mechanically large: ~21 methods, ~9 calling files, needs doing one entity type at a time with a working build/test after each to avoid a big-bang break. Tracked as its own task rather than folded into the smaller leak-fixing work.

**Revised effort read:** the *scalar/property* layer of the model (the bulk of `clsCharacter.cs`/`clsOptions.cs` by line count) is genuinely close to portable as originally hoped. The *collection/creation* layer (`clsUnique.cs`, `clsEquipment.cs`) is not - the TreeNode-building above is a real, separate refactor that needs its own budget, not a rounding error on top of the property-layer cleanup.
