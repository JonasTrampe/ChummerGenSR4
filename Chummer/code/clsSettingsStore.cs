...
namespace Chummer
{
	...
	public static class SettingsStore
	{
		...
		public static SettingsRegistryKey CurrentUser => new SettingsRegistryKey();
		...
		private static Dictionary<string, Dictionary<string, string>> Load()
		{
			...
			try
			{
				XDocument objDocument = XDocument.Load(SettingsFilePath);
				foreach (XElement objKeyElement in objDocument.Root?.Elements("key") ?? Enumerable.Empty<XElement>())
				{
					string strPath = objKeyElement.Attribute("path")?.Value;
					if (string.IsNullOrEmpty(strPath))
						continue;

					var dicValues = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
					foreach (XElement objValueElement in objKeyElement.Elements("value"))
					{
						string strName = objValueElement.Attribute("name")?.Value;
		...
		private static void MigrateFromWindowsRegistry()
		{
			...
			try
			{
				bool blnMigratedAnything = false;
				foreach (string strSubKeyPath in LegacySubKeyPaths)
				{
					RegistryKey objRegistryKey = Registry.CurrentUser.OpenSubKey(strSubKeyPath);
					if (objRegistryKey == null)
						continue;

					var dicValues = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
					foreach (string strValueName in objRegistryKey.GetValueNames())
					{
						object objValue = objRegistryKey.GetValue(strValueName);
						if (objValue != null)
						{
		...
		internal static string GetValue(string strSubKeyPath, string strName)
		{
			lock (LockObject)
			{
				return Data.TryGetValue(strSubKeyPath, out var dicValues) && dicValues.TryGetValue(strName, out var strValue)
					? strValue
					: null;
			}
		}
	...
	public sealed class SettingsRegistryKey
	{
		...
		internal string SubKeyPath { get; private set; } = string.Empty;
